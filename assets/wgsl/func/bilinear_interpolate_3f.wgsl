fn bilinear_interpolate_3f(uv: vec2<f32>) -> vec3<f32> {
  ...
  return src_3f(minX, minY) * ((1.0 - fx) * (1.0 - fy)) +
         src_3f(minX, minY + 1) * ((1.0 - fx) * fy) +
         src_3f(minX + 1, minY) * (fx * (1.0 - fy)) +
         src_3f(minX + 1, minY + 1) * (fx * fy);
}