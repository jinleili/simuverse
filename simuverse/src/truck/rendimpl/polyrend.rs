    ...
    fn buffers(
        ...
        (
            BufferHandler::from_slice(
                ...
                device,
                vertex_usage,
                Some("PolygonMesh vertices"),
            ),
            BufferHandler::from_slice(&indices, device, index_usage, Some("PolygonMesh indices")),
        )
    }
}
...
impl ToInstance<WireFrameInstance> for PolygonMesh {
    ...
    fn to_instance(
        ...
        let vb = BufferHandler::from_slice(
            ...
            device,
            BufferUsages::VERTEX,
            Some("polygon mesh pos"),
        );
        let ib = BufferHandler::from_slice(
            &strips,
            device,
            BufferUsages::INDEX,
            Some("polygon mesh strips"),
        );
        WireFrameInstance {
            vertices: Arc::new(vb),
            strips: Arc::new(ib),
    ...
    fn buffers(
        ...
        (
            BufferHandler::from_slice(
                ...
                device,
                vertex_usage,
                Some("StructuredMesh vertices"),
            ),
            BufferHandler::from_slice(
                &indices,
                device,
                index_usage,
                Some("StructuredMesh indices"),
            ),
        )
    }
}
...
impl ToInstance<WireFrameInstance> for StructuredMesh {
    ...
    fn to_instance(
        ...
        let vb = BufferHandler::from_slice(
            ...
            device,
            BufferUsages::VERTEX,
            Some("StructuredMesh pos"),
        );
        let ib = BufferHandler::from_slice(
            &strips,
            device,
            BufferUsages::INDEX,
            Some("StructuredMesh strips"),
        );
        WireFrameInstance {
            vertices: Arc::new(vb),
            strips: Arc::new(ib),
...
impl ToInstance<WireFrameInstance> for PolylineCurve<Point3> {
    ...
    fn to_instance(
        ...
        let vb = BufferHandler::from_slice(
            ...
            device,
            BufferUsages::VERTEX,
            Some("polyline curve positions"),
        );
        let ib = BufferHandler::from_slice(
            &strips,
            device,
            BufferUsages::INDEX,
            Some("polyline curve strips"),
        );
        WireFrameInstance {
            vertices: Arc::new(vb),
            strips: Arc::new(ib),
...
impl ToInstance<WireFrameInstance> for Vec<PolylineCurve<Point3>> {
    ...
    fn to_instance(
        ...
        let strips: Vec<u32> = self
            ...
            .collect();
        let vb =
            BufferHandler::from_slice(&positions, device, BufferUsages::VERTEX, Some("positions"));
        let ib = BufferHandler::from_slice(&strips, device, BufferUsages::INDEX, Some("strips"));
        WireFrameInstance {
            vertices: Arc::new(vb),
            strips: Arc::new(ib),
            state: state.clone(),
