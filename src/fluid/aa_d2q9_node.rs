...
impl AAD2Q9Node {
    pub fn new(
        ...
        println!("AAD2Q9Node dispatch_group_count: {:?}", dispatch_group_count);
        ...
        let collide_stream_shader = create_shader_module(
            device,
        ...
        queue.write_buffer(&dynamic_buf.buffer, 0, bytemuck::cast_slice(&dynamic_data0));
        queue.write_buffer(
            &dynamic_buf.buffer,
            dynamic_offset,
            bytemuck::cast_slice(&dynamic_data1),
        );
    ...
    pub fn dispatch<'c, 'b: 'c>(&'b self, cpass: &mut wgpu::ComputePass<'c>, _swap_index: usize) {
        self.collide_stream_node
            .dispatch_by_offsets(cpass, Some(vec![vec![0], vec![256]]));
    }
}
